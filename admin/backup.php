...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_sqlite) {
    ...
    elseif ($action === 'repair_db') {
        try {
            ...
            // 定义核心表结构
            $schema = [
                'admin_users' => "CREATE TABLE admin_users (id INTEGER PRIMARY KEY AUTOINCREMENT, username VARCHAR(50) NOT NULL UNIQUE, password VARCHAR(255) NOT NULL, email VARCHAR(100), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, last_login TIMESTAMP)",
                'system_config' => "CREATE TABLE system_config (id INTEGER PRIMARY KEY AUTOINCREMENT, config_key VARCHAR(100) NOT NULL UNIQUE, config_value TEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
                'announcements' => "CREATE TABLE announcements (id INTEGER PRIMARY KEY AUTOINCREMENT, title VARCHAR(255) NOT NULL, content TEXT NOT NULL, is_pinned BOOLEAN DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
                'selectable_numbers' => "CREATE TABLE selectable_numbers (id INTEGER PRIMARY KEY AUTOINCREMENT, number VARCHAR(20) NOT NULL UNIQUE, is_premium BOOLEAN DEFAULT 0, status VARCHAR(20) DEFAULT 'available', used_at TIMESTAMP, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
                'icp_applications' => "CREATE TABLE icp_applications (id INTEGER PRIMARY KEY AUTOINCREMENT, number VARCHAR(20) NOT NULL, website_name VARCHAR(100) NOT NULL, domain VARCHAR(100) NOT NULL, description TEXT, owner_name VARCHAR(50), owner_email VARCHAR(100), status VARCHAR(20) DEFAULT 'pending', reject_reason TEXT, payment_platform VARCHAR(50), transaction_id VARCHAR(255), is_resubmitted BOOLEAN DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, reviewed_at TIMESTAMP, reviewed_by INTEGER, FOREIGN KEY (reviewed_by) REFERENCES admin_users(id))",
                'login_attempts' => "CREATE TABLE login_attempts (id INTEGER PRIMARY KEY AUTOINCREMENT, ip_address VARCHAR(45) NOT NULL, username VARCHAR(100), attempt_time DATETIME DEFAULT CURRENT_TIMESTAMP, success BOOLEAN DEFAULT 0, user_agent TEXT)",
                'email_verifications' => "CREATE TABLE email_verifications (id INTEGER PRIMARY KEY AUTOINCREMENT, application_id INTEGER NOT NULL, email VARCHAR(100) NOT NULL, code VARCHAR(10) NOT NULL, expires_at DATETIME NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)",
                'plugins' => "CREATE TABLE plugins (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(100) NOT NULL, identifier VARCHAR(50) NOT NULL UNIQUE, version VARCHAR(20), description TEXT, author VARCHAR(100), is_active BOOLEAN DEFAULT 0, installed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
                'migrations' => "CREATE TABLE migrations (id INTEGER PRIMARY KEY AUTOINCREMENT, version VARCHAR(255) NOT NULL UNIQUE, applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
            ];
